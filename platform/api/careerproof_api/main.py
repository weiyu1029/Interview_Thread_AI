...
@app.get("/health")
def health() -> dict:
    return {"status": "ok", "service": "interviewthread-api", "version": app.version}
...
async def send_chat_message(
    ...
    if thread.provider == "deterministic":
        ...
        messages = [
            {"role": "system", "content": f"Use only this InterviewThread evidence and never fabricate claims. Respond fully in locale {payload.locale}, except for source quotations and proper nouns: {evidence}"},
            *history,
