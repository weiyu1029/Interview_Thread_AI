  ...
  privacy: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "We use service providers needed to operate the site, including Cloudflare infrastructure and D1, the identity provider you choose, ElevenLabs when cloud read-aloud is configured and requested, Microsoft Azure Speech for signed-in cloud transcript correction and read-aloud fallback, and a transactional email provider for contact-form delivery. Data also goes to an external endpoint only when you select a feature that requires it. We do not sell personal data, build advertising profiles, share career evidence with employers, or use the service to make employment decisions.",
        ],
      },
      {
...
const traditionalChinesePages: Record<InformationPageKey, InformationPageCopy> = {
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: "7. 分享與販售", paragraphs: ["我們使用營運網站所需的服務商，包括 Cloudflare 基礎設施與 D1、你選擇的身分提供者、在你要求雲端朗讀時使用的 ElevenLabs、用於已登入雲端逐字稿校正與朗讀備援的 Microsoft Azure Speech，以及負責寄送聯絡表單的交易郵件服務商。只有當你主動選擇需要外部端點的功能時，資料才會送往該端點。我們不販售個人資料、不建立廣告画像、不把職涯證據分享給雇主，也不使用本服務做出聘僱決策。"] },
