...
const traditionalChinesePages: Record<InformationPageKey, InformationPageCopy> = {
  ...
  privacy: {
    ...
    sections: [
      ...
      { title: "4. 回饋、活動與封測申請資料", paragraphs: ["若你主動送出意見回饋，我們會保存分類、評分、訊息、語言、狀態、時間、帳號擁有者、產品版本、功能頁面，以及適用時的封測梯次。登入後也可能保存有限事件，例如完成分析、開始或回答面試、更新追蹤器、送出回饋、申請或退出封測。事件只表示操作曾發生，不包含履歷、職缺描述、答案逐字稿或原始語音。", "若你申請封閉測試，我們會保存結構化的職類、資歷、面試時程、主要需求、語言、梯次狀態、分開選擇的研究／更新同意，以及你接受的條款、隱私與產品版本。封測申請不要求履歷或自由書寫的職涯歷史；你可以退出封測而不刪除帳號。"] },
      ...
      { title: "7. 分享與販售", paragraphs: ["我們使用營運網站所需的服務商，包括 Cloudflare 基礎設施與 D1，以及你選擇的身分提供者。只有當你主動選擇需要外部端點的功能時，資料才會送往該端點。我們不販售個人資料、不建立廣告画像、不把職涯證據分享給雇主，也不使用本服務做出聘僱決策。"] },
